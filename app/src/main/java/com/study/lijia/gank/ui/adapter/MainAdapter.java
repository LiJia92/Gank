...
public class MainAdapter extends BaseQuickAdapter<DataModel, BaseViewHolder> {

    public MainAdapter(@Nullable List<DataModel> data) {
    ...
    protected void convert(BaseViewHolder helper, DataModel item) {
        if (item.results.restList != null
            ...
            DataModel.ItemData restData = item.results.restList.get(0);
